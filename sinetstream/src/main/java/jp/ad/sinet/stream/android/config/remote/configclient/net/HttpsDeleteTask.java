...
public class HttpsDeleteTask extends AlternateAsyncTask<Integer> {
    private final String TAG = HttpsDeleteTask.class.getSimpleName();

    private final WeakReference<Context> mWeakReference;
    private final String mUrlString;
    private final HttpsDeleteTaskListener mListener;
    ...
    public HttpsDeleteTask(@NonNull Context context,
                           @NonNull String urlString,
                           @NonNull HttpsDeleteTaskListener listener) {
                ...
                switch (result) {
                    case HttpURLConnection.HTTP_OK:
                        if (mResponseData != null) {
                            mListener.onDeleteFinished(mResponseData);
                        } else if (mResponseArray != null) {
                            mListener.onDeleteFinished(mResponseArray);
                        } else {
    ...
    private void setConnectionParameters(
        ...
        try {
            ...
            mErrMsg = "HttpsUrlConnection.setRequestMethod(" +
                    HTTP_METHOD_DELETE + "): " + e.getMessage();
            Log.w(TAG, mErrMsg);
            return;
        }
    ...
    private int downloadJsonData(@NonNull URL url) {
            ...
            switch (responseCode) {
                case HttpURLConnection.HTTP_OK:
                    ...
                    if (responseJsonString != null && responseJsonString.isEmpty()) {
                        /* For DELETE method, response message can have "empty body". */
                        if (mDebugEnabled) {
                            Log.d(TAG, "Empty body, OK");
                        }
                        break;
                    }
                    break;
                ...
                case HttpURLConnection.HTTP_NOT_FOUND:
                default:
                    mErrMsg = HTTP_METHOD_DELETE + " " + mUrlString + ":\n";
        ...
        if (mErrMsg == null && responseJsonString != null && !responseJsonString.isEmpty()) {
    ...
    public interface HttpsDeleteTaskListener {
        void onDeleteFinished(@NonNull JSONObject responseData);
        void onDeleteFinished(@NonNull JSONArray responseData);
